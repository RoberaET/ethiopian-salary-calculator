...
const SalaryVisualization = lazy(() => 
  import("@/components/salary-visualization").then(module => ({ 
    default: module.SalaryVisualization 
  })).catch(() => ({ 
    default: () => <div className="p-8 text-center text-muted-foreground">Chart unavailable</div> 
  }))
)

const DarkVeil = lazy(() => 
  import("@/components/dark-veil").then(module => ({ 
    default: module.default 
  })).catch(() => ({ 
    default: () => <div className="w-full h-full bg-gradient-to-br from-gray-900 to-black" /> 
  }))
)

const SalaryNegotiationMode = lazy(() => 
  import("@/components/salary-negotiation-mode").then(module => ({ 
    default: module.SalaryNegotiationMode 
  })).catch(() => ({ 
    default: () => <div className="p-4 text-center text-muted-foreground">Negotiation mode unavailable</div> 
  }))
)

const CurrencyConverter = lazy(() => 
  import("@/components/currency-converter").then(module => ({ 
    default: module.CurrencyConverter 
  })).catch(() => ({ 
    default: () => <div className="p-4 text-center text-muted-foreground">Currency converter unavailable</div> 
  }))
)

const WhatIfCalculator = lazy(() => 
  import("@/components/what-if-calculator").then(module => ({ 
    default: module.WhatIfCalculator 
  })).catch(() => ({ 
    default: () => <div className="p-4 text-center text-muted-foreground">What-if calculator unavailable</div> 
  }))
)

const ExportShareOptions = lazy(() => 
  import("@/components/export-share-options").then(module => ({ 
    default: module.ExportShareOptions 
  })).catch(() => ({ 
    default: () => <div className="p-4 text-center text-muted-foreground">Export options unavailable</div> 
  }))
)
